...
export default function Clubs() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(err.error || `Failed to create club: ${response.statusText}`);
      }

      const data = await response.json();
      setClubs([...clubs, data.data]);
      setFormData({ displayName: '', status: 'unknown' });
    } catch (err) {
  ...
  const handleDeleteConfirm = async () => {
    if (deleteModal.clubIndex !== null) {
      const club = clubs[deleteModal.clubIndex];
      try {
        const response = await fetch(`/api/clubs/${club.uid}`, { method: 'DELETE' });
        if (!response.ok) {
          const err = await response.json().catch(() => ({}));
          throw new Error(err.error || `Failed to delete club: ${response.statusText}`);
        }
    ...
    setDeleteModal({ isOpen: false, clubIndex: null, clubName: '' });
  };
  ...
  const handleEditConfirm = async () => {
    if (editModal.clubIndex !== null) {
      const club = clubs[editModal.clubIndex];
      try {
        const response = await fetch(`/api/clubs/${club.uid}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        if (!response.ok) {
          const err = await response.json().catch(() => ({}));
          throw new Error(err.error || `Failed to update club: ${response.statusText}`);
        }

        const data = await response.json();
        const updatedClubs = [...clubs];
        updatedClubs[editModal.clubIndex] = data.data;
    ...
    setEditModal({ isOpen: false, clubIndex: null, formData: { displayName: '', status: 'unknown' } });
  };
